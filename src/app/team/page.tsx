...
const Team = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-900 text-white font-nunito">
      <h1 className="text-4xl md:text-6xl font-bold mb-8 text-yellow-400">Meet the Team</h1>
      <div className="flex flex-col md:flex-row items-center space-y-8 md:space-y-0 md:space-x-8">
        {teamMembers.map((member, index) => (
          <div key={index} className="bg-gray-800 p-6 rounded-lg shadow-lg text-center w-72 h-72 flex flex-col justify-center">
            <h2 className="text-xl font-semibold mb-2 text-yellow-400">{member.title}</h2>
            <h3 className="text-lg font-medium mb-4 text-white">{member.name}</h3>
